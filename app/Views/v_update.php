...
<h1>Update Barang</h1>
    <?php if ($data_barang): ?>
        ...
        <input type="number" id="Harga" name="Harga" value="<?= $data_barang['Harga'] ?>">

        <label for="Foto">Foto</label>
        <input type="file" id="Foto" name="Foto">
        <?php if ($data_barang['Foto']): ?>
        <p>Foto saat ini: <img src="<?= base_url('path/to/upload/directory/' . $data_barang['Foto']); ?>" width="150" height="150"></p>
        <?php endif; ?>

        <button type="submit" class="btn btn-update" onclick="return confirmUpdate()">Update</button>
    </form>
    <?php else: ?>
...
<script>
    function confirmUpdate() {
        ...
        // Kembalikan false agar form tidak di-submit secara otomatis
        return false;
    }
</script>
